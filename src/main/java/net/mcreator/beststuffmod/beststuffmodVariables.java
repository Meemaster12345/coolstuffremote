package net.mcreator.beststuffmod;
...
public class beststuffmodVariables {
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "beststuffmod_mapvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				beststuffmod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				beststuffmod.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldVariables extends WorldSavedData {
		public static final String DATA_NAME = "beststuffmod_worldvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				beststuffmod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				beststuffmod.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage> {
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world) {
			if (context.side == Side.SERVER) {
				if (message.type == 0)
					beststuffmod.PACKET_HANDLER.sendToAll(message);
				else
					beststuffmod.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
